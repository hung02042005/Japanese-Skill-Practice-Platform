...
**Trạng thái (12/07/2026):** P0 xong (trừ backup off-VPS, hoãn có chủ đích). P1 xong 3/4 (P1.7 chờ chủ dự án cấp SSH public key từng người). P2 xong 3/4 (P2.11 blue-green chủ đích chưa làm — xem lý do ở bảng dưới). Toàn bộ đã verify qua CI/CD thật, không chỉ code.
...
| P0.2 — Backup DB tự động | 🟡 Gần xong (quyết định hoãn có chủ đích, tái xác nhận 12/07/2026) | Script (`/opt/scripts/backup-db.sh`) + systemd timer (`backup-db.timer`, 3h sáng hàng ngày, giữ 14 bản gần nhất) đã chạy thật trên VPS, **đã test restore thành công** (27 bảng, 14 dòng `student_users` khớp dữ liệu sống, container/volume test đã dọn dẹp sạch). **Chưa làm — hoãn theo yêu cầu chủ dự án (11/07 và tái xác nhận 12/07/2026):** đẩy bản backup ra ngoài VPS (rclone/S3/Google Drive...), cần credential cloud storage mà AI không tự có. **Rủi ro còn tồn tại:** nếu VPS hỏng ổ cứng/mất toàn bộ, backup nằm cùng máy cũng mất theo — ưu tiên làm nốt phần này trước khi coi P0 hoàn tất 100%. |
...
| P2.8 — Uptime monitoring | 🟢 Xong (bản DIY miễn phí, verify qua domain thật) | `nginx.conf` thêm `location = /health` -> proxy `backend:8080/actuator/health` (chỉ đúng 1 path, không mở cả `/actuator/*`). `.github/workflows/uptime-check.yml` (`schedule: */10 * * * *` + `workflow_dispatch`) gọi `https://sakuji.online` + `/health` mỗi 10 phút qua chính domain thật (sau Cloudflare) — fail job (báo đỏ + email mặc định của GitHub) nếu 1 trong 2 không đúng. Đã verify: `curl https://sakuji.online/health` → `{"status":"UP"}` sau deploy thật. **Giới hạn đã ghi rõ:** đây KHÔNG thay thế hoàn toàn UptimeRobot/Healthchecks.io (không SMS/đa kênh, GitHub không đảm bảo giờ cron tuyệt đối) — cần tài khoản email của chủ dự án nên AI không tự đăng ký hộ được; khuyến nghị làm thêm khi cần độ tin cậy cao hơn. |
| P2.9 — Nâng ngưỡng JaCoCo coverage | 🟢 Xong (bước đầu tiên, đo bằng số thật) | Đo coverage thật bằng `mvn clean verify` cục bộ (12/07/2026): 61 test pass, **17.9% LINE coverage** (247 class). Nâng ngưỡng `pom.xml` từ `0.10` → `0.15` — tăng thật so với cũ, vẫn chừa ~3 điểm % đệm dưới mức đo được để không đỏ CI vì 1 thay đổi nhỏ. Verify: `mvn clean verify` pass với ngưỡng mới + CI thật (GitHub Actions) cũng pass. Lần nâng tiếp theo nên đo lại số thật tương tự, không đoán. |
| P2.10 — Bật `npm run test` trong CI | 🟢 Xong | Tooling test (`vitest`, `jsdom`, `@testing-library/react`, `src/test/setup.js`) đã có sẵn từ trước nhưng **0 file test tồn tại** và bước Test trong `ci.yml` bị comment — viết 2 file test đầu tiên: `apiMessage.test.js` (6 test, logic ưu tiên message lỗi: backend > Network Error > `err.message` > fallback) và `authSlice.test.js` (6 test, reducer `loginThunk` pending/fulfilled/rejected + `clearError` + `logout` — kiểm cả việc `logout` xoá đúng localStorage, không chỉ Redux state). Cả 2 test thuần logic/reducer, không cần mock axios hay render component. 12/12 test pass cục bộ + lint sạch + CI thật (GitHub Actions) pass. Bước `Test` trong `ci.yml` đã bỏ comment. |
| P2.11 — Rolling update/blue-green | ⚪ Chưa làm (chủ đích, đúng theo kế hoạch) | Xếp cuối cùng vì tốn công nhất (cần proxy/load balancer biết chuyển traffic động) trong khi P1.4 (staging) + P1.5 (rollback nhanh) đã giảm phần lớn rủi ro deploy — chỉ đáng làm khi vài chục giây downtime/lần deploy thực sự ảnh hưởng traffic thật. Không triển khai bây giờ để tránh tối ưu sớm (premature optimization). |
