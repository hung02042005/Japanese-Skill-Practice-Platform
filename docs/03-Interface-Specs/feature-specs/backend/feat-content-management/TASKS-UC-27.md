...
> **Cập nhật:** 2026-06-12 | **Lưu ý:** Media chỉ lưu URL.

## Phase 1: Database & Domain

- [ ] 1.1 Tận dụng index sẵn (`IX_vocab_*`, `IX_kanji_public_level`) (NFR-27-01). KHÔNG sửa cột.
- [ ] 1.2 Entity `Vocabulary`/`Kanji` map đủ cột + workflow; `Lesson` chỉ read-only; enum `JlptLevel`.

## Phase 2: Repository Layer

- [ ] 2.1 `Vocabulary/KanjiRepository.findByCreatedByWithFilters(...)` — loại `deleted`.
...
- [ ] 3.3 `Vocabulary/KanjiDetailResponse` — không lộ Entity (NFR-27-04).
...
- [ ] 4.5 Submit-review qua `ContentSubmissionService` (`contentType∈{vocabulary,kanji}`): re-validate theo loại + guard status → `pending_review` (FR-27-25/26).
- [ ] 4.6 SLF4J log `[INFO] Staff {id} {action} {contentType} {cid}` (FR-27-08); soft delete (NFR-27-07).

## Phase 5: Controller & Security

- [ ] 5.1 `StaffLearningContentController` (`/api/staff/vocabulary`, `/api/staff/kanji`): create/update.
- [ ] 5.2 `@PreAuthorize(...)`; map 400/401/403/404/409 theo UC-27 §7.

## Phase 6: Testing & QA

- [ ] 6.1 Unit `LearningContentService` (kanji trùng, vocab furigana, state-machine, ownership) — `feat-testing/TC-UC-27`.
- [ ] 6.2 Integration: media URL (không BLOB), `character_value` UNIQUE.
- [ ] 6.3 API/MockMvc: 201/401/403/404/409; DTO.

## Phase 7: Frontend

- [ ] 7.1 `LearningContentPage` (tab vocabulary/kanji; upload trả URL; form theo loại).

## Phase 8: Final Review

- [ ] 8.1 Cross-check `UC-27` §3 (FR-27-01..08, FR-27-16..26) ↔ code; 10 AC còn lại có test phủ (`TRACEABILITY.md`).
