...
- [ ] 1.3 Rà soát Entity `Question`, `GrammarPoint`, `Vocabulary`, `Kanji`, `Assessment`, `QuestionAssignment`, `AttemptAnswer`, `StaffUser` có map đủ cột workflow (`status`, `created_by`, `approved_by`, `published_at`, `created_at`, `updated_at`); `Lesson` chỉ cần mapping read-only cho các liên kết hiện có.
...
- [ ] 2.4 `GrammarPointRepository`, `VocabularyRepository`, `KanjiRepository`: `findByCreatedByWithFilters(...)`; `KanjiRepository.existsByCharacterValue(...)` (FR-27-21); `LessonRepository` chỉ đọc để kiểm tra liên kết.
...
- [ ] 3.3 Request DTOs UC-27: `CreateVocabularyRequest` (FR-27-16), `CreateKanjiRequest` (≥1 onyomi/kunyomi — FR-27-20).
...
- [ ] 3.5 Response DTOs: `QuestionSummaryResponse`/`QuestionDetailResponse` (+`isLocked`), `Grammar/Vocabulary/KanjiDetailResponse`, `AssessmentSummaryResponse`/`AssessmentDetailResponse` (+`assignedScoreSum`, `scoreMatched`, `sections[]`/`questions[]`), `AssignResultResponse`, `SubmitReviewResponse`, `PageResponse<T>` — KHÔNG lộ Entity (ADR-005).
...
- [ ] 4.4 `LearningContentService`: vocabulary/kanji create+update; trùng kanji → `KanjiDuplicateException`; media chỉ lưu URL (ADR-006) — UC-27 FR-27-16..24.
...
- [ ] 5.3 `StaffLearningContentController` (`/api/staff/vocabulary`, `/api/staff/kanji`): create/update.
...
- [ ] 7.3 `GrammarEditorPage` + `LearningContentPage` (tab vocabulary/kanji, upload trả URL).
