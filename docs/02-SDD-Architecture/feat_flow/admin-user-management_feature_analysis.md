...
> **Không bao gồm**: `AdminDashboardController`/`AdminDashboardService` (UC-36 Dashboard), `AdminSettingsController` (UC-39 Settings — đã có `admin-system_feature_analysis.md`), `AdminAuditLogController` (xem log), `AdminNotificationRuleController`, `MaintenanceModeService`, `DebugController` — tất cả đều nằm trong package `com.jlpt.feature.admin` nhưng không được `AdminController`/`AdminUserService` gọi tới, nên nằm ngoài phạm vi tài liệu này.
...
10. **Tài liệu "Staff Students Page" riêng** (`feat-staff-student-management_feature_analysis.md`) được người giao nhiệm vụ xác nhận là đang soạn ở nơi khác — tài liệu này chỉ đối chiếu ở mức cần thiết để trả lời câu hỏi kiến trúc (mục 1, mục 5.7), không phân tích đầy đủ 8 mục cho feature đó.
