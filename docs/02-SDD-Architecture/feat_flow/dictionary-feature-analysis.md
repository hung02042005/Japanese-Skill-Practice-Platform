...
> Hai tính năng anh em xem [flashcard-feature-analysis.md](flashcard-feature-analysis.md) và [notebook-feature-analysis.md](notebook-feature-analysis.md).
>
...
4. Bấm "Lưu vào sổ" (chỉ Từ vựng) → `saveToNotebook('VOCABULARY', id)` → `POST /api/notebook/words` với `reason: 'manual'` (xem [notebook-feature-analysis.md](notebook-feature-analysis.md)).
...
| *(nối sang Sổ tay)* `NotebookService.addWrongWordsToReviewDeck` | xem [notebook-feature-analysis.md](notebook-feature-analysis.md) §4.2 | `flashcards`, `flashcard_decks` | ✍️ |
