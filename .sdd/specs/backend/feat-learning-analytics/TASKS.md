...
- [ ] 1.1 Tạo `dto/response/AnalyticsResponse.java`
...
- [ ] 1.2 Tạo `dto/response/StreakDetailResponse.java`
  - `currentStreak`, `longestStreak`, `lastActivityDate`, `streakStatus`, `streakStatusMessage`
- [ ] 1.3 Tạo `dto/response/CompletionRateResponse.java`
  - `contentType`, `totalCount`, `completedCount`, `completionRate`
- [ ] 1.4 Tạo `dto/response/QuizStatsResponse.java`
...
- [ ] 1.5 Tạo `dto/response/ExamAttemptResponse.java`
  - `attemptId`, `attemptType`, `assessmentTitle`, `totalScore`, `maxScore`, `isPassed`, `startedAt`, `submittedAt`
- [ ] 1.6 Tạo `dto/response/DashboardResponse.java`
...
- [ ] 1.7 Tạo `dto/response/AdminReportResponse.java`
...
- [ ] 2.1 Tạo `service/AnalyticsService.java`
...
- [ ] 2.2 Implement `getStudentProgressAnalytics(Long studentId)`
...
- [ ] 2.3 Implement `getStudyStreakDetail(Long studentId)`
  - Tính `streakStatus` và `streakStatusMessage` tương ứng
  - Múi giờ: `LocalDate.now()` tại server
- [ ] 2.4 Implement `getCompletionRate(Long studentId, String contentType)`
  - `contentType == null` → tổng hợp tất cả types
  - Xử lý chia cho 0 → trả `completionRate = 0`
- [ ] 2.5 Implement `getQuizStats(Long assessmentId)`
...
- [ ] 2.6 Implement `getStudentExamHistory(Long studentId, int page, int size)`
  - Query `testAttempts` theo `studentId + type in ('exam','quiz')`, order `startedAt DESC`
- [ ] 2.7 Implement `getAdminDashboard()` — chỉ ADMIN
...
- [ ] 2.8 Implement `getStaffDashboard(Long staffId)` — chỉ STAFF
...
- [ ] 2.9 Implement `getAdminReport(LocalDate startDate, LocalDate endDate)`
...
- [ ] 2.10 Implement `exportReport(LocalDate startDate, LocalDate endDate, String format)`
...
- [ ] 3.1 Tạo `controller/student/StudentAnalyticsController.java`
...
- [ ] 3.2 Thêm Staff/Admin endpoints vào cùng controller hoặc `StaffAnalyticsController`:
...
- [ ] 3.3 Verify: Student gọi `GET /analytics/students/{otherId}/exam-history` → `403` (không được xem của người khác)
...
- [ ] 4.1 Unit test `AnalyticsServiceTest`:
...
- [ ] 4.2 Integration test `StudentAnalyticsControllerTest`:
...
- [ ] 4.3 Integration test Dashboard:
