/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   Mô tả: Schema đầy đủ cho hệ thống học tiếng Nhật JLPT (Đã tái cấu trúc tách vai trò)
   Đáp ứng: 20 UC Student + 12 UC Staff + 2 UC StaffManager + 6 UC Admin
...
/* ============================================================================
   I. NHÓM BẢNG: NGƯỜI DÙNG & XÁC THỰC (Đã tách riêng biệt theo vai trò)
   (Phục vụ: UC-01, UC-02, UC-03, UC-04, UC-05, UC-18, UC-20, UC-22, UC-30, UC-32)
   ============================================================================ */
...
CREATE TABLE admin_users (
    ...
    password_hash        NVARCHAR(255)   NULL,                       -- NULL khi đăng nhập OAuth (nếu có)
...
CREATE TABLE student_users (
    ...
    email                NVARCHAR(255)   NOT NULL UNIQUE,
    password_hash        NVARCHAR(255)   NULL,                       -- NULL khi đăng nhập OAuth
    full_name            NVARCHAR(150)   NOT NULL,
    status               NVARCHAR(20)    NOT NULL DEFAULT 'active'
        CHECK (status IN ('active','suspended','pending','deleted')),
    suspend_reason       NVARCHAR(500)   NULL,
    ...
    -- Hồ sơ mở rộng (Tích hợp từ user_profiles)
    ...
    -- Thống kê Streak (Tích hợp từ user_streaks)
...
/* ============================================================================
   ...
   (Phục vụ: UC-06, UC-07, UC-08, UC-09, UC-14, UC-15, UC-16, UC-24, UC-30 Staff)
   ============================================================================ */
...
CREATE TABLE lessons (
    ...
    content_text     NVARCHAR(MAX)   NULL,                      -- text bài giảng
    video_url        NVARCHAR(500)   NULL,                      -- YouTube / Vimeo
    audio_url        NVARCHAR(500)   NULL,
    attachment_url   NVARCHAR(500)   NULL,                      -- PDF đính kèm
...
/* ============================================================================
   ...
   (Phục vụ: UC-10, UC-11, UC-14, UC-15, UC-23, UC-26, UC-27, UC-31)
   ============================================================================ */

-- 12. Ngân hàng câu hỏi - UC-23
CREATE TABLE questions (
    ...
    audio_url        NVARCHAR(500)  NULL,                       -- cho câu hỏi nghe
    ...
    created_by       BIGINT         NULL,                       -- staff tạo
    status           NVARCHAR(20)   NOT NULL DEFAULT 'draft'
        CHECK (status IN ('draft','pending_review','rejected','published','archived','deleted')),
    approved_by      BIGINT         NULL,
    published_at     DATETIME2      NULL,
...
-- 14. Bài kiểm tra Quiz / Exam - UC-26, UC-31
...
-- 15. Gán câu hỏi cho Assessment / Reading-Listening-Speaking Material
CREATE TABLE question_assignments (
    ...
    section_name    NVARCHAR(100)   NULL,                       -- phần thi: 'Đọc hiểu', 'Nghe'...
    score           DECIMAL(6,2)    NOT NULL DEFAULT 1,         -- điểm cho câu này
    display_order   INT             NOT NULL DEFAULT 0,
...
/* ============================================================================
   ...
   (Phục vụ: UC-10, UC-11, UC-13, UC-14, UC-15, UC-27, UC-56)
   ============================================================================ */

-- 17. Lần làm bài (quiz / exam / practice / reading / listening) của Học viên
CREATE TABLE test_attempts (
    ...
    parent_id         BIGINT          NULL,                     -- NULL nếu là random_practice
...
-- 18. Đáp án từng câu mà học viên đã chọn
CREATE TABLE attempt_answers (
    ...
    answer_text        NVARCHAR(MAX)   NULL,                    -- cho điền khuyết
    is_correct         BIT             NULL,
...
-- 19. Bài nộp của học viên - compact cho Speaking/Handwriting/Manual grading
CREATE TABLE student_submissions (
    ...
    exercise_id       BIGINT          NULL,                    -- lessons(lesson_type='speaking')
...
/* ============================================================================
   V. NHÓM BẢNG: TIẾN ĐỘ HỌC TẬP - FLASHCARD - BOOKMARK (Liên kết Học viên)
   (Phục vụ: UC-06, UC-07, UC-08, UC-09, UC-12, UC-17, UC-19)
   ============================================================================ */

-- 20. Tiến độ học và bookmark (học viên)
...
/* ============================================================================
   ...
   (Phục vụ: UC-21, UC-28, UC-42)
   ============================================================================ */

-- 23. Ticket hỗ trợ - UC-21
CREATE TABLE tickets (
    ticket_id       BIGINT IDENTITY(1,1) PRIMARY KEY,
    student_id      BIGINT          NOT NULL,                   -- học viên gửi
    ...
    assigned_to     BIGINT          NULL,                       -- staff được giao
...
CREATE TABLE notifications (
    ...
    is_auto           BIT             NOT NULL DEFAULT 0,       -- gửi từ rule tự động
    rule_key          NVARCHAR(100)   NULL,                     -- key cá»§a notification rule
    scheduled_at      DATETIME2       NULL,
    sent_at           DATETIME2       NULL,
    is_read           BIT             NOT NULL DEFAULT 0,
    read_at           DATETIME2       NULL,
    delivered_at      DATETIME2       NULL,
    admin_creator_id  BIGINT          NULL,                     -- admin tạo
    staff_creator_id  BIGINT          NULL,                     -- staff tạo
    created_at        DATETIME2       NOT NULL DEFAULT SYSUTCDATETIME(),
    ...
    CONSTRAINT CK_noti_creator CHECK (
        ...
        (admin_creator_id IS NULL AND staff_creator_id IS NULL) -- Tự động hệ thống
    )
);
GO
...
/* ============================================================================
   ...
   (Phục vụ: UC-31 Admin, UC-40, UC-41, UC-42)
   ============================================================================ */

-- 27. Cấu hình hệ thống - UC-41 (key-value)
CREATE TABLE system_settings (
    ...
    updated_by      BIGINT          NULL,                       -- Admin cập nhật
    updated_at      DATETIME2       NOT NULL DEFAULT SYSUTCDATETIME(),
...
-- 28. Audit log thao tác Admin/Staff/StaffManager (Tạo tin, cập nhật cấu hình, đổi vai trò...)
CREATE TABLE admin_audit_logs (
    audit_id        BIGINT IDENTITY(1,1) PRIMARY KEY,
    admin_actor_id  BIGINT          NULL,                       -- Admin thực hiện
    staff_actor_id  BIGINT          NULL,                       -- Staff thực hiện
...
/* ============================================================================
   VIII. VIEW THỐNG KÊ (Đã tối ưu hóa, loại bỏ hoàn toàn các liên kết JOIN cồng kềnh)
   (Phục vụ: UC-19, UC-19 Staff, UC-31, UC-40)
   ============================================================================ */

-- View: Thống kê học tập của từng Học viên
...
SELECT
    ...
    -- Đã hoàn thành (truy vấn trực tiếp)
...
/* ============================================================================
   IX. DỮ LIỆU MẪU - CẤU HÌNH HỆ THỐNG
   ============================================================================ */
...
PRINT N'✅ Đã khởi tạo hoàn tất cơ sở dữ liệu JLPT_LearningDB cải tiến (Tách vai trò người dùng + Tối ưu hóa 3NF).';
GO
