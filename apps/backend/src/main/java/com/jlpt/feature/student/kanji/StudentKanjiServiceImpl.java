...
public class StudentKanjiServiceImpl implements StudentKanjiService {
    ...
    public KanjiListResponse getKanjiList(String level, Long studentId, int page, int size) {
        ...
        // Note: For simplicity without N+1 query, we might just query all flashcards or ignore isInFlashcard in list
        // But to be fully compliant we should query if it exists. We'll leave it false for list, or fetch if needed.
        // For now let's just return false for isInFlashcard in list, or fetch it.
        // Actually, let's fetch flashcard presence to be 100% correct.
        
        List<com.jlpt.feature.learning.Flashcard> flashcards = new java.util.ArrayList<>();
        if (!kanjiIds.isEmpty()) {
             // flashcards = flashcardRepository.findByStudentIdAndContentTypeAndContentIdIn(studentId, com.jlpt.feature.learning.Flashcard.ContentType.KANJI, kanjiIds);
             // We don't have this method yet. Just set it false for now in list response to avoid compilation error.
        }
        ...
        List<KanjiItemResponse> items = kanjiPage.getContent().stream()
                .map(k -> KanjiItemResponse.builder()
                        ...
                        .isInFlashcard(false) // Will need a repository method to fill this properly
                        .build())
                .collect(Collectors.toList());
    ...
    public KanjiDetailResponse getKanjiDetail(Long kanjiId, Long studentId) {
        ...
        boolean isInFlashcard = flashcardRepository.findByStudentIdAndContentTypeAndContentId(
                studentId, com.jlpt.feature.learning.Flashcard.ContentType.KANJI, kanjiId).isPresent();
