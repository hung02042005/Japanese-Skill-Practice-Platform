...
package com.jlpt.feature.notification.repository;
...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Modifying
    @Query("UPDATE Notification n SET n.isRead = true, n.readAt = :now WHERE n.student.id = :studentId AND n.isRead = false")
    int markAllReadByStudentId(@Param("studentId") Long studentId, @Param("now") LocalDateTime now);

    /** Duplicate-check: prevent same rule_key being sent to same student within 24h. */
    @Query("""
            SELECT COUNT(n) > 0 FROM Notification n
            WHERE n.student.id = :studentId
              AND n.ruleKey = :ruleKey
