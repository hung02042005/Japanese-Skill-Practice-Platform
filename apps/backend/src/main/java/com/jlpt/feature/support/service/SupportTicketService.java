...
public class SupportTicketService {
    ...
    // ── UC-30: Get my notifications (Student) ───────────────────────────────
    ...
    // ── UC-30: Broadcast notification (Staff — async) ───────────────────────
    ...
    public Page<com.jlpt.feature.support.dto.SubmissionResponse> getAllSubmissions(
        ...
        var type = submissionType == null || submissionType.isBlank()
                ? null
                : com.jlpt.feature.corelearning.entity.StudentSubmission.SubmissionType.valueOf(submissionType.toUpperCase());
        var parsedStatus = status == null || status.isBlank()
                ? null
                : com.jlpt.feature.corelearning.entity.StudentSubmission.SubmissionStatus.valueOf(status.toUpperCase());
    ...
    private com.jlpt.feature.support.dto.SubmissionResponse toSubmissionResponse(
            com.jlpt.feature.corelearning.entity.StudentSubmission s) {
    ...
    public GradeResponse manualGrade(Long submissionId, String actorEmail, ManualGradeRequest req) {
        ...
        if (submission.getSubmissionType() != com.jlpt.feature.corelearning.entity.StudentSubmission.SubmissionType.SPEAKING) {
            throw new BusinessException(422, "INVALID_SUBMISSION_TYPE",
                    "Chỉ có thể chấm điểm thủ công bài nói (speaking)");
        }
        if (submission.getStatus() != com.jlpt.feature.corelearning.entity.StudentSubmission.SubmissionStatus.AI_GRADED) {
        ...
        submission.setStatus(com.jlpt.feature.corelearning.entity.StudentSubmission.SubmissionStatus.GRADED);
        submissionRepository.save(submission);

        notificationRepository.save(Notification.builder()
                .student(submission.getStudent())
                .title("Bài nói của bạn đã được chấm điểm")
                .content("Điểm của bạn: " + req.getManualScore() + "/100")
