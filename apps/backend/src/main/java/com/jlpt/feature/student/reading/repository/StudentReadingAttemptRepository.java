...
public interface StudentReadingAttemptRepository extends JpaRepository<TestAttempt, Long> {
    ...
    @Query(
            "SELECT DISTINCT ta.parentId FROM TestAttempt ta WHERE ta.student.id = :studentId AND ta.parentType = :parentType AND ta.parentId IN :parentIds AND ta.attemptType = :attemptType")
