...
function blankForm(prefill) {
  return {
    ...
    optionA:       prefill?.options?.A    ?? '',
    optionB:       prefill?.options?.B    ?? '',
    optionC:       prefill?.options?.C    ?? '',
    optionD:       prefill?.options?.D    ?? '',
    correctOption: prefill?.correctOption ?? '',
    correctAnswer: prefill?.correctAnswer ?? '',
...
function validate(form) {
  ...
  if (form.questionType === 'fill_blank') {
    if (!form.correctAnswer.trim()) errors.correctAnswer = 'Vui lòng nhập đáp án đúng.';
  }
...
/* ── QuestionFormModal ───────────────────────────────────────────── */
export default function QuestionFormModal({ isOpen, editQuestion, prefillData, onClose, onSave }) {
  ...
  const handleSave = (status) => {
    ...
    const savedQ = {
      ...
      status,
      isLocked:     editQuestion?.isLocked ?? false,
      ...(options ? { options, correctOption: form.correctOption } : {}),
      ...(form.questionType !== 'multiple_choice' ? { correctAnswer: form.correctAnswer.trim() } : {}),
    };
        ...
        <div className="sfq-modal-body">
          ...
          {form.questionType === 'multiple_choice' && (
            <>
              <div className="sfq-options-grid">
                ...
                {/* Option C */}
                <div className="sfq-field">
                  <label className="sfq-field-label">Đáp án C</label>
                  <input
                    type="text"
                    className="sfq-input"
                    placeholder="Nhập đáp án C (tuỳ chọn)"
                    value={form.optionC}
                ...
                {/* Option D */}
                <div className="sfq-field">
                  <label className="sfq-field-label">Đáp án D</label>
                  <input
                    type="text"
                    className="sfq-input"
                    placeholder="Nhập đáp án D (tuỳ chọn)"
                    value={form.optionD}
              ...
              <div className="sfq-field">
                ...
                <div className="sfq-radio-group">
                  {['A', 'B', 'C', 'D'].map((letter) => {
                    ...
                    if (letter === 'C' && !optVal && !form.optionC) return null;
                    if (letter === 'D' && !optVal && !form.optionD) return null;
                    return (
          ...
          {form.questionType === 'fill_blank' && (
            <div className="sfq-field">
              ...
              <input
                type="text"
                className={`sfq-input${errors.correctAnswer ? ' sfq-input--err' : ''}`}
                placeholder="Nhập đáp án đúng"
                value={form.correctAnswer}
                onChange={set('correctAnswer')}
              />
              {errors.correctAnswer && (
                <span className="sfq-field-error">{errors.correctAnswer}</span>
              )}
            </div>
          )}
              ...
              <div className="sfq-radio-group">
                <label className="sfq-radio-label">
                  <input
                    ...
                    name="correctAnswer"
                    value="Đúng"
                    checked={form.correctAnswer === 'Đúng'}
                    onChange={set('correctAnswer')}
                  />
                  Đúng
                </label>
                <label className="sfq-radio-label">
                  <input
                    ...
                    name="correctAnswer"
                    value="Sai"
                    checked={form.correctAnswer === 'Sai'}
                    onChange={set('correctAnswer')}
                  />
                  Sai
                </label>
