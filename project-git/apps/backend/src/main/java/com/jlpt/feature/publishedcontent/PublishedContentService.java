...
public class PublishedContentService {
    ...
    public PublishedContentListResponse getPublishedContents(
        ...
        List<ManagedContentSnapshot> all = new ArrayList<>();
        if (StringUtils.hasText(typeStr)) {
            all.addAll(resolver.resolve(ContentType.fromValue(typeStr)).findPublished(level));
        } else {
            for (ManagedContentHandler handler : resolver.distinctHandlers()) {
                all.addAll(handler.findPublished(level));
            }
        }
        ...
        List<PublishedContentItemResponse> content =
    ...
    @Transactional(readOnly = true)
    public PublishedContentDetailResponse getContentDetail(String managerEmail, Long contentId, String typeStr) {
        requireManager(managerEmail);
    ...
    private PublishedContentItemResponse toItem(ManagedContentSnapshot s) {
        return PublishedContentItemResponse.builder()
                .contentId(s.getContentId())
                .contentType(s.getContentType().getValue())
                .titleOrText(s.getTitleOrText())
                .jlptLevel(s.getJlptLevel())
                .status(s.getStatus())
